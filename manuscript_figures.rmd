...
chem_year <- df %>%
  ...
  geom_vline(xintercept = 2004)+
     annotate("text",
           ...
           angle  = 90,
           size   = theme.size)+
    scale_fill_manual(values = min_pal_qual2[c(3:10,2, 1)],
                      labels = label_order)+
  theme_classic()+
  labs(x    = "Year", 
...
bis_ana <- df %>% 
  ...
  select(!chem_name_shiny) %>% 
  unique() %>%
  complete(subclass, year = 1990:2020, fill = list(`n()` = 0)) %>%
  ...
  labs(x = "Year", 
       y = NULL, 
       fill = "")+
   scale_fill_manual(values = c(min_pal_qual2[3], "#203448"),
                     labels = bis.label)+
...
chem_time2 <- wrap_plots(A = (chem_year + ggtitle("A. Published articles per chemical class over time") + title_theme), 
                         B = (pht_ana   + ggtitle("B. Phthalates and phthalate substitutes") + title_theme),
                         C = (pbde_ope  + ggtitle("C. PBDEs and OPEs") + title_theme),
                         D = (bis_ana   + ggtitle("D. BPA and bisphenol analogues") + title_theme), 
                         design = wrp2) 
...
#pop_color <- min_pal_qual2[c(6,8,7)]
 pop_color <- c('Pre- and perinatal'  = min_pal_qual2[8],
                "Pre-adult (<18 yrs)" = min_pal_qual2[9],
                'Adult (>18yrs)'      = min_pal_qual2[10]
                    )
...
pop_exp <- df_pop %>% 
  ...
  ggplot(
    ...
    scale_x_continuous(breaks=c(0, 500,1000,1500,2000, 2500), limits=c(0, 2750)
                       )+
    labs(x    = "Number of articles", 
         y    = "",
         fill = "")+
...
study_risk_plot <- (pop_gene          + ggtitle("A. Population type") + title_theme) +
                   (pop_gend          + ggtitle("B. Biological sex") + title_theme) + 
                   (pop_exp           + ggtitle("C. Age at exposure assessment") + title_theme) +
                   (pop_health        + ggtitle("D. Age at health outcome assessment") + title_theme) +
...
chem_index <- readxl::read_excel(here("Data", "final_chem_shiny.xlsx"), 
                                 sheet = "Additives"
                                 ) %>% janitor::clean_names()
poly_index <- readxl::read_excel(here("Data", "final_chem_shiny.xlsx"), 
                                 sheet = "Polymers"
                                 ) %>% janitor::clean_names()
...
dfa <- df %>%
      ...
      mutate(ci_code = to_code(country_investigated),
             c2_code = to_code(country_first_auth),
             c3_code = to_code(country_last_author)
             ) %>%
      distinct(refid, 
               ci_code, 
               c2_code, 
...
check <- df %>% 
  ...
  group_by(country_investigated,
           code) %>%
  count() %>% 
  left_join(country_meta2, by = c("code" = "code"))
...
# require(svglite)
# ggsave(here("Figures", "Chemicals over time.svg"),chem_year, dpi = 600, height = 100, width = 188, units="mm")
# # population plot
# #ggsave(here("figures", "population_plot.png"),population_plot,  dpi = 600, height = 200, width = 250, units="mm")
# #study design plot
# ggsave(here("figures", "study_risk_plot.svg"),study_risk_plot,  dpi = 600, height = 200, width = 188, units="mm")
# #world map
# ggsave(here("figures", "world_map_populations2.svg"),world_maps2,  dpi = 600, height = 200, width = 188, units="mm")
# #heatmap ages
# #ggsave(here("figures", "Heatmap_ages.png"),heatmap_age_assess,  dpi = 600, height = 200, width = 200, units="mm")
# #heatmap 
# ggsave(here("figures", "heatmap_HOM_EXP2.svg"), Hom_exp_map,  dpi = 600, height = 250, width = 188, units="mm")
# #iceberg
